...
7. 更新下第2条，为什么对于语言和图像模型都说输出是从概率分布中采样呢？对于LM来说，sequence of tokens 本来就是从模型的概率分布p中 sample 出来的，用条件概率来评价这个序列的好坏。而对于VM，以分类任务为例，最终输出的类别分数被 Softmax 归一化得到了一个概率分布。使用交叉熵作为损失函数，即优化目标，衡量模型的预测分布和真实分布之间的差异。
``` python
# calculate the loss
output = net(imgs)
